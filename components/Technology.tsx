...
function WaveSVG() {
  return (
    <svg viewBox="0 0 400 180" className="w-full h-full" fill="none">
      <motion.path
        d="M0,90 Q40,0 80,90 T160,90 T240,90 T320,90 T400,90"
        stroke="#B87333"
        strokeOpacity="0.2"
        strokeWidth="1.5"
...
const cards = [
  ...
  },
  {
    ...
    body: 'Interactive nodes exert a radial gravitational pull on the stabilized cursor. As the pointer enters a nodes influence radius, it is drawn organically toward the center - transforming strenuous fine-clicks into effortless gestures. The pull curve is configurable per element.',
    metric: '60px',
  ...
  return (
    <section ref={ref} id="technology" className="relative bg-bone-dim" style={{ height: `${cards.length * 100}vh` }}>
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        {cards.map((card, i) => (
          <StackingCard key={i} card={card} index={i} total={cards.length} scrollYProgress={scrollYProgress} />
        ))}
      </div>
    </section>
  );
...
function CardProgress({ scrollYProgress, start, end }: { scrollYProgress: MotionValue<number>; start: number; end: number }) {
  const width = useTransform(scrollYProgress, [start, end], ['0%', '100%']);
  return <motion.div className="absolute top-0 left-0 h-full bg-copper" style={{ width }} />;
}
