...
export function EditorialMarketing({ onStart }: ModernMarketingProps) {
  ...
  return (
    <div ref={containerRef} className="bg-paper text-ink min-h-screen font-sans selection:bg-teal selection:text-paper cursor-none">
      ...
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${navSolid ? "bg-paper/80 backdrop-blur-md border-b border-line" : "bg-transparent"}`}
      >
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-4 flex items-center justify-between">
          <a href="#" className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono uppercase tracking-[0.2em] text-ink-muted hidden sm:inline">/ accessibility substrate</span>
          </a>
          <div className="hidden md:flex items-center gap-8 text-sm text-ink-soft">
            <a href="#playground" className="hover:text-teal transition-colors">Playground</a>
            <a href="#technology" className="hover:text-teal transition-colors">Technology</a>
          </div>
          <button onClick={onStart} className="group relative px-4 py-2 bg-ink text-paper text-xs font-medium rounded-sm overflow-hidden">
      ...
      <main className="flex-grow w-full relative z-10">
        ...
        <section ref={heroRef} className="relative min-h-screen pt-32 pb-20 px-6 lg:px-12 flex flex-col justify-center">
          <motion.div style={{ y, opacity, scale }} className="max-w-[1400px] mx-auto w-full">
            <div className="flex items-center justify-between mb-16 lg:mb-24">
              <div className="flex items-center gap-3 text-[11px] font-mono uppercase tracking-[0.2em] text-ink-muted">
                <span className="w-2 h-2 rounded-full bg-teal animate-pulse"></span>
                Open Source · v0.4.2
              </div>
              <div className="hidden md:flex items-center gap-6 text-[11px] font-mono uppercase tracking-[0.2em] text-ink-muted">
                <span>01 — Intent</span>
                <span className="w-8 h-px bg-line"></span>
                <span>02 — Substrate</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-6 mb-16">
              <div className="col-span-12 lg:col-span-8">
                <h1 className="font-serif text-[clamp(2.8rem,8vw,7.5rem)] leading-[0.95] tracking-tighter text-ink">
                  Your intent,<br />not your <span className="italic font-light text-teal">tremor.</span>
                </h1>
              </div>
              <div className="col-span-12 lg:col-span-4 lg:pt-4 flex flex-col justify-end">
                ...
                <div className="mt-8 flex items-center gap-4">
                  <button onClick={onStart} className="group relative px-6 py-3 bg-ink text-paper text-sm font-medium rounded-sm overflow-hidden">
        ...
        {/* Playground */}
        <section id="playground" className="w-full px-6 lg:px-12 py-24 max-w-[1400px] mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            <div className="lg:col-span-4 flex flex-col gap-12">
              <h2 className="font-serif text-4xl tracking-tighter text-ink">The Math</h2>
              <div className="flex flex-col gap-8">
                <div>
                  <h3 className="text-sm font-mono uppercase tracking-wider text-teal mb-3">01. Algorithmic Dampening</h3>
                ...
                <div>
                  <h3 className="text-sm font-mono uppercase tracking-wider text-amber-soft mb-3">02. Magnetic Geometry</h3>
                  <p className="text-sm leading-relaxed text-ink-soft">
            ...
            <div className="lg:col-span-8">
              <div className="relative">
                <div ref={arenaRef} className="relative w-full h-[420px] rounded-sm border border-line bg-paper-warm overflow-hidden">
                  <canvas ref={canvasRef} className="absolute inset-0"></canvas>
                  
                  <div className="absolute top-4 left-4 flex items-center gap-2 text-[10px] font-mono uppercase tracking-[0.2em] text-ink-muted">
                    <span className={`w-1.5 h-1.5 rounded-full ${hovering ? 'bg-teal animate-pulse' : 'bg-ink-muted/40'}`}></span>
                    {hovering ? 'Live · Stabilizing' : 'Hover to activate'}
                  </div>
                  
                  <div className="absolute top-4 right-4 text-[10px] font-mono uppercase tracking-[0.2em] text-ink-muted tnum">
                    {hitCount}/{targetCount} targets
                  </div>

                  <div className="absolute bottom-4 left-4 flex items-center gap-4 text-[10px] font-mono uppercase tracking-[0.15em] text-ink-muted">
                    <span className="flex items-center gap-1.5">
                      <span className="w-3 h-px bg-amber"></span> Raw signal
                    </span>
                    <span className="flex items-center gap-1.5">
                      <span className="w-3 h-px bg-teal"></span> Stabilized
                    </span>
                  </div>
                </div>
              </div>
